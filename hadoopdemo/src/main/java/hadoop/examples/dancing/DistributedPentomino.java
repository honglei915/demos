...
package hadoop.examples.dancing;
...
public class DistributedPentomino extends Configured implements Tool {
	...
	public static class PentMap extends
		...
		class SolutionCatcher implements
			...
			public void solution(List<List<Pentomino.ColumnName>> answer) {
				String board = Pentomino.stringifySolution(width, height,
						answer);
				try {
		...
		public void setup(Context context) {
			...
			pent = (Pentomino) ReflectionUtils.newInstance(
					conf.getClass(Pentomino.CLASS, OneSidedPentomino.class),
					conf);
	 ...
	 */
	private static long createInputDirectory(FileSystem fs, Path dir,
		...
		PrintWriter file = new PrintWriter(new OutputStreamWriter(
				new BufferedOutputStream(fs.create(input), 64 * 1024),
				Charsets.UTF_8));

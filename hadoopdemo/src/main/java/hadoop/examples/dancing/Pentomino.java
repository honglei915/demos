			...
			if (rotate % 2 == 0) {
				...
				for (int y = 0; y < height; ++y) {
					...
					for (int x = 0; x < width; ++x) {
						result[y][x] = shape[doFlip(flipY, y, height)][doFlip(
								flipX, x, width)];
					}
				}
			} else {
				...
				for (int y = 0; y < height; ++y) {
					result[y] = new boolean[width];
					for (int x = 0; x < width; ++x) {
						result[y][x] = shape[doFlip(flipX, x, width)][doFlip(
								flipY, y, height)];
					}
				}
			}
			...
			for (int x = 0; x < width; ++x) {
				for (int y = 0; y < height; ++y) {
					if (y + shape.length <= height
							...
							&& (!upperLeft || (isSide(x, shape[0].length, width) && isSide(
									y, shape.length, height)))) {

 ...
 */
package hadoop.examples.terasort;
 ...
 * reference that it be odd. There is also a facility for quickly advancing the
 * state of the generator by a fixed number of steps - this facilitates parallel
 * generation. This is based on 1.0 of rand16.c from Chris Nyberg
...
class Random16 {
	 ...
	 * mod 2**128 All structs after the first contain an "a" and "c" that
	 * comprise the square of the previous function. f**2(x) = (Gen[1].a * x +
	 * Gen[1].c) mod 2**128 f**4(x) = (Gen[2].a * x + Gen[2].c) mod 2**128

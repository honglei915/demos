 ...
 * Pi, consider using distbbp. Reference: [1] David H. Bailey, Peter B. Borwein
 * and Simon Plouffe. On the Rapid Computation of Various Polylogarithmic
 * Constants. Math. Comp., 66:903-913, 1996.
 */
public class BaileyBorweinPlouffe extends Configured implements Tool {
	...
	private static final Log LOG = LogFactory
			.getLog(BaileyBorweinPlouffe.class);
	...
	public static class BbpReducer extends
		...
		@Override
		protected void reduce(LongWritable offset,
		...
		protected void cleanup(Context context) throws IOException,
			...
			if (conf.getInt(DIGIT_START_PROPERTY, 1) == 1) {
				...
				try {
					...
					out.println("Total number of hexadecimal digits is " + 2
							* hex.size() + ".");
					...
					out.println("Total number of decimal digits is "
							+ decDigits + ".");
				} finally {
					outputstream.close();
				}
	...
	private static <T> void print(PrintWriter out, Iterator<T> iterator,
			String prefix, String format, int elementsPerGroup,
			int groupsPerLine) {
	 ...
	 */
	public static class BbpInputFormat extends
			InputFormat<LongWritable, IntWritable> {
		...
		public List<InputSplit> getSplits(JobContext context) {
			...
			for (int i = 0; i < parts.length; ++i) {
				final int k = i < parts.length - 1 ? parts[i + 1] : nDigits
						+ startDigit - 1;
	...
	public int run(String[] args) throws IOException {
		...
		if (startDigit <= 0) {
			throw new IllegalArgumentException("startDigit = " + startDigit
					+ " <= 0");
		} else if (nDigits <= 0) {
			...
			throw new UnsupportedOperationException(
					"nDigits - 1 + startDigit = " + (nDigits - 1L + startDigit)
							+ " > IMPLEMENTATION_LIMIT + BBP_HEX_DIGITS,"
							+ ", where IMPLEMENTATION_LIMIT="
							+ IMPLEMENTATION_LIMIT + "and BBP_HEX_DIGITS="
							+ BBP_HEX_DIGITS);
		} else if (nMaps <= 0) {
	...
	static int[] partition(final int offset, final int size, final int nParts) {
		...
		for (int i = 1; i < nParts; i++) {
			final long target = offset + i * (total / nParts) + i
					* (total % nParts) / nParts;
			...
			for (; high > low + 4;) {
				final int mid = (high + low - 2 * remainder) / 8 * 4
						+ remainder;
			...
			parts[i] = high == low ? high : workload(high) - target > target
	...
	private static long workload(final long n) {
		if (n < 0) {
			...
			throw new IllegalArgumentException("n = " + n + " > MAX_N = "
					+ MAX_N);
		}

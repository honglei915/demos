...
public final class DistSum extends Configured implements Tool {
	...
	static class Parameters {
		...
		private Parameters(Machine machine, int nThreads, int nJobs,
				int nParts, String remoteDir, File localDir) {
		...
		static Parameters parse(String[] args, int i) {
			if (args.length - i < COUNT)
				throw new IllegalArgumentException("args.length - i < COUNT = "
						+ COUNT + ", args.length=" + args.length + ", i=" + i
			...
			if (!"m".equals(type) && !"r".equals(type) && !"x".equals(type)) {
				throw new IllegalArgumentException("type=" + type
						+ " is not equal to m, r or x");
			} else if (nParts <= 0) {
				throw new IllegalArgumentException("nParts = " + nParts
						+ " <= 0");
			} else if (nJobs <= 0) {
			...
			return new Parameters("m".equals(type) ? MapSide.INSTANCE
					: "r".equals(type) ? ReduceSide.INSTANCE
							: MixMachine.INSTANCE, nThreads, nJobs, nParts,
					remoteDir, localDir);
		}
	}
	...
	public static abstract class Machine {
		...
		public static final class SummationSplit extends InputSplit implements
				Writable, Container<Summation> {
		...
		public static abstract class AbstractInputFormat extends
				InputFormat<NullWritable, SummationWritable> {
	...
	public static class MapSide extends Machine {
		...
		public static class PartitionInputFormat extends AbstractInputFormat {
			...
			public List<InputSplit> getSplits(JobContext context) {
				...
				final Summation sigma = SummationWritable.read(DistSum.class,
						conf);
		...
		public static class SummingMapper
			...
			@Override
			protected void map(NullWritable nw, SummationWritable sigma,
	...
	public static class ReduceSide extends Machine {
		...
		public static class PartitionMapper
			...
			protected void map(NullWritable nw, SummationWritable sigma,
				...
				for (int i = 0; i < parts.length; ++i) {
					context.write(new IntWritable(i), new SummationWritable(
							parts[i]));
					LOG.info("parts[" + i + "] = " + parts[i]);
				}
			}
		...
		public static class IndexPartitioner extends
				Partitioner<IntWritable, SummationWritable> {
			/** Return the index as the partition. */
			@Override
			public int getPartition(IntWritable index, SummationWritable value,
					int numPartitions) {
				return index.get();
			}
		}
		...
		public static class SummingReducer
			...
			protected void reduce(IntWritable index,
					Iterable<SummationWritable> sums, Context context)
					throws IOException, InterruptedException {
	...
	public static class MixMachine extends Machine {
		...
		public synchronized void init(Job job) throws IOException {
			final Configuration conf = job.getConfiguration();
			if (cluster == null) {
				String jobTrackerStr = conf.get("mapreduce.jobtracker.address",
						"localhost:8012");
				cluster = new Cluster(NetUtils.createSocketAddr(jobTrackerStr),
						conf);
		...
		private Machine chooseMachine(Configuration conf) throws IOException {
			...
			try {
				for (;; Thread.sleep(2000)) {
					...
					final int m = status.getMapSlotCapacity()
							- status.getOccupiedMapSlots();
					final int r = status.getReduceSlotCapacity()
							- status.getOccupiedReduceSlots();
					if (m >= parts || r >= parts) {
						// favor ReduceSide machine
						final Machine value = r >= parts ? ReduceSide.INSTANCE
								: MapSide.INSTANCE;
						Util.out.println("  " + this + " is " + value + " (m="
								+ m + ", r=" + r + ")");
						return value;
					}
				}
	...
	private void compute(final String name, Summation sigma) throws IOException {
		...
		final String startmessage = "steps/parts = " + sigma.E.getSteps() + "/"
				+ parameters.nParts + " = "
		...
		public Computation call() {
			if (sigma.getValue() == null)
				try {
					...
					Util.out.println("ERROR: Got an exception from "
							+ getJobName());
	...
	private Summation execute(String name, Summation sigma) {
		...
		final List<Summation> combined = Util
				.combine(Arrays.asList(summations));

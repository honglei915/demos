...
public class TaskResult implements Container<Summation>,
	...
	public TaskResult combine(TaskResult that) {
		...
		return s == null ? null : new TaskResult(s, this.duration
				+ that.duration);
	}
	...
	public static TaskResult valueOf(String s) {
		...
		if (j < 0)
			throw new IllegalArgumentException("i=" + i + ", j=" + j
					+ " < 0, s=" + s);
		final Summation sigma = Summation.valueOf(Util.parseStringVariable(
				"sigma", s.substring(i, j)));
		...
		if (j < 0)
			throw new IllegalArgumentException("i=" + i + ", j=" + j
					+ " < 0, s=" + s);
		final long duration = Util.parseLongVariable("duration",
				s.substring(i, j));

...
class LongLong {
	...
	BigInteger toBigInteger() {
		return BigInteger.valueOf(d1).shiftLeft(BITS_PER_LONG)
				.add(BigInteger.valueOf(d0));
	}
	...
	public String toString() {
		...
		return String.format("%x*2^%d + %016x", d1 << remainder, BITS_PER_LONG
				- remainder, d0);
	}
	...
	static LongLong multiplication(final LongLong r, final long a, final long b) {
		 ...
		 * >> MID; final long t = (x0 + x1)*(y0 + y1); final long u = (x0 -
		 * x1)*(y0 - y1); final long v = x1*y1; final long tmp = (t - u)>>>1;
		 * result.d0 = ((t + u)>>>1) - v + ((tmp << MID) & FULL_MASK);;
		 * result.d1 = v + (tmp >> MID); return result;
		 */

 ...
 */
package hadoop.examples.pi.math;

import hadoop.examples.pi.Combinable;
import hadoop.examples.pi.Container;
import hadoop.examples.pi.Util;
...
public class Summation implements Container<Summation>, Combinable<Summation> {
	...
	/** Constructor */
	Summation(long valueN, long deltaN, long limitN, long valueE, long deltaE,
			long limitE) {
	...
	public static Summation valueOf(final String s) {
		...
		if (j < 0)
			throw new IllegalArgumentException("i=" + i + ", j=" + j
					+ " < 0, s=" + s);
		final ArithmeticProgression N = ArithmeticProgression.valueOf(s
				.substring(i, j));
		...
		if (j < 0)
			throw new IllegalArgumentException("i=" + i + ", j=" + j
					+ " < 0, s=" + s);
		final ArithmeticProgression E = ArithmeticProgression.valueOf(s
				.substring(i, j));

		final Summation sigma = new Summation(N, E);
		i = j + 1;
		if (s.length() > i) {
			final String value = Util.parseStringVariable("value",
					s.substring(i));
			sigma.setValue(value.indexOf('.') < 0 ? Double
					.longBitsToDouble(Long.parseLong(value)) : Double
					.parseDouble(value));
		}
		return sigma;
	}
	...
	public double compute() {
		if (value == null)
			value = N.limit <= MAX_MODULAR ? compute_modular()
					: compute_montgomery();
		return value;
	}
	...
	public Summation combine(Summation that) {
		...
		if (this.E.limit == that.E.value && this.N.limit == that.N.value) {
			...
			final Summation s = new Summation(new ArithmeticProgression(
					N.symbol, N.value, N.delta, that.N.limit),
					new ArithmeticProgression(E.symbol, E.value, E.delta,
							that.E.limit));
			s.setValue(v);
			return s;
		}
	...
	public <T extends Container<Summation>> List<Summation> remainingTerms(
			List<T> sorted) {
		...
		if (sorted != null)
			for (Container<Summation> c : sorted) {
				...
				final Summation s = new Summation(sigma.N.limit, N.delta,
						remaining.N.limit, sigma.E.limit, E.delta,
						remaining.E.limit);
				if (s.getSteps() > 0)
					results.add(s);
				remaining = new Summation(remaining.N.value, N.delta,
						sigma.N.value, remaining.E.value, E.delta,
						sigma.E.value);
			}
	...
	public Summation[] partition(final int nParts) {
		...
		for (int i = 1; i < parts.length; i++) {
			...
			parts[i - 1] = new Summation(new ArithmeticProgression(N.symbol,
					prevN, N.delta, currN), new ArithmeticProgression(E.symbol,
					prevE, E.delta, currE));
		...
		parts[parts.length - 1] = new Summation(new ArithmeticProgression(
				N.symbol, prevN, N.delta, N.limit), new ArithmeticProgression(
				E.symbol, prevE, E.delta, E.limit));
		return parts;
	}
}
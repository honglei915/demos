 ...
 * +1/(10n+9)) References: [1] David H. Bailey, Peter B. Borwein and Simon
 * Plouffe. On the Rapid Computation of Various Polylogarithmic Constants. Math.
 * Comp., 66:903-913, 1996. [2] Fabrice Bellard. A new formula to compute the
 * n'th binary digit of pi, 1997. Available at http://fabrice.bellard.free.fr/pi
 * .
 */
public final class Bellard {
	...
	public enum Parameter {
		 ...
		 * (10k + 9) )
		 */
		P20_21(true, 1, 20, 2), P20_3(false, 3, 20, 0), P20_5(false, 5, 20, -4), P20_7(
				false, 7, 20, -4), P20_9(true, 9, 20, -6), P20_11(P20_21), P20_13(
				P20_3), P20_15(P20_5), P20_17(P20_7), P20_19(P20_9);
		...
		public static Parameter get(String s) {
			...
			throw new IllegalArgumentException("s=" + s + ", parts="
					+ Arrays.asList(parts));
		}
	}

	/** The sums in the Bellard's formula */
	public static class Sum implements Container<Summation>,
			Iterable<Summation> {
		...
		/** Constructor */
		private <T extends Container<Summation>> Sum(long b, Parameter p,
				int nParts, List<T> existing) {
			if (b < 0)
		...
		private static <T extends Container<Summation>> Summation[] partition(
			...
			if (existing == null || existing.isEmpty())
				...
				final List<Summation> remaining = sigma
						.remainingTerms(existing);
	...
	public static <T extends Container<Summation>> double computePi(
		...
		if (results.size() != Parameter.values().length)
			throw new IllegalArgumentException(
					"m.size() != Parameter.values().length" + ", m.size()="
							+ results.size() + "\n  m=" + results);

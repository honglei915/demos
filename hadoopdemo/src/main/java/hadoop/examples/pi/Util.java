...
import hadoop.examples.pi.DistSum.Machine;
...
public class Util {
	...
	public static class Timer {
		...
		public synchronized long tick(String s) {
			...
			if (s != null) {
				out.format("%15dms (=%-15s: %s%n", delta, millis2String(delta)
						+ ")", s);
				out.flush();
			}
			previous = t;
	...
	public static String parseStringVariable(final String name, final String s) {
		if (!s.startsWith(name + '='))
			throw new IllegalArgumentException(
					"!s.startsWith(name + '='), name=" + name + ", s=" + s);
	...
	public static <T, E extends Callable<T>> void execute(int nThreads,
			List<E> callables) throws InterruptedException, ExecutionException {
	...
	public static PrintWriter createWriter(File dir, String prefix)
			throws IOException {
		checkDirectory(dir);
		...
		for (;;) {
			...
			if (!f.exists())
				return new PrintWriter(new OutputStreamWriter(
						new FileOutputStream(f), Charsets.UTF_8));
	...
	public static void printBitSkipped(final long b) {
		out.println();
		out.println("b = " + long2string(b) + " (" + (b < 2 ? "bit" : "bits")
				+ " skipped)");
	}
	...
	public static String pi2string(final double pi, final long terms) {
		...
		return String.format("%0" + acc_hex + "X %0" + (13 - acc_hex)
				+ "X (%d hex digits)", value >> shift, value
				& ((1 << shift) - 1), acc_hex);
	}
	...
	public static int accuracy(final long terms, boolean print) {
		final double error = terms <= 0 ? 2
				: (Math.log(terms) / Math.log(2)) / 2;
		final int bits = MACHEPS_EXPONENT - (int) Math.ceil(error);
		if (print)
			out.println("accuracy: bits=" + bits + ", terms="
	...
	/** Run a job. */
	static void runJob(String name, Job job, Machine machine,
			String startmessage, Util.Timer timer) {
		...
		try {
			try {
				...
				// Separate jobs
				final long sleeptime = 1000L * job.getConfiguration().getInt(
						JOB_SEPARATION_PROPERTY, 10);
				if (sleeptime > 0) {
					Util.out.println(name + "> sleep("
							+ Util.millis2String(sleeptime) + ")");
			...
			if (starttime != null)
				timer.tick(name + "> timetaken="
						+ Util.millis2String(timer.tick() - starttime));
		}
	}

	/** Read job outputs */
	static List<TaskResult> readJobOutputs(FileSystem fs, Path outdir)
			throws IOException {
	...
	/** Write results */
	static void writeResults(String name, List<TaskResult> results,
			FileSystem fs, String dir) throws IOException {
		final Path outfile = new Path(dir, name + ".txt");
		Util.out.println(name + "> writing results to " + outfile);
		final PrintWriter out = new PrintWriter(new OutputStreamWriter(
				fs.create(outfile), Charsets.UTF_8), true);
		try {
	...
	/** Create a directory. */
	static boolean createNonexistingDirectory(FileSystem fs, Path dir)
			throws IOException {
		if (fs.exists(dir)) {

 ...
 * Pi is obtained once $I$ is evaluated numerically. There are better methods
 * for computing Pi. We emphasize numerical approximation of arbitrary integrals
 * in this example. For computing many digits of Pi, consider using bbp. The
 * implementation is discussed below. Mapper: Generate points in a unit square
 * and then count points inside/outside of the inscribed circle of the square.
 * Reducer: Accumulate points inside/outside results from the mappers. Let
 * numTotal = numInside + numOutside. The fraction numInside/numTotal is a
...
public class QuasiMonteCarlo extends Configured implements Tool {
	...
	static private final String TMP_DIR_PREFIX = QuasiMonteCarlo.class
			.getSimpleName();
	...
	public static class QmcMapper extends
		...
		public void map(LongWritable offset, LongWritable size, Context context)
			...
			context.write(new BooleanWritable(true),
					new LongWritable(numInside));
			context.write(new BooleanWritable(false), new LongWritable(
					numOutside));
		}
	}
	...
	public static BigDecimal estimatePi(int numMaps, long numPoints,
			Path tmpDir, Configuration conf) throws IOException,
		...
		try {
			...
			for (int i = 0; i < numMaps; ++i) {
				...
				final SequenceFile.Writer writer = SequenceFile.createWriter(
						fs, conf, file, LongWritable.class, LongWritable.class,
						CompressionType.NONE);
				try {
			...
			final BigDecimal numTotal = BigDecimal.valueOf(numMaps).multiply(
					BigDecimal.valueOf(numPoints));

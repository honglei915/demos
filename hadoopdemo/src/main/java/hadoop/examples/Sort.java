 ...
 * bin/hadoop jar build/hadoop-examples.jar sort [-r <i>reduces</i>] [-inFormat
 * <i>input format class</i>] [-outFormat <i>output format class</i>] [-outKey
 * <i>output key class</i>] [-outValue <i>output value class</i>] [-totalOrder
 * <i>pcnt</i> <i>num samples</i> <i>max splits</i>] <i>in-dir</i>
 * <i>out-dir</i>
 */
public class Sort<K, V> extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; ++i) {
			try {
				if ("-r".equals(args[i])) {
					...
					sampler = new InputSampler.RandomSampler<K, V>(pcnt,
		...
		System.out.println("Running on " + cluster.getTaskTrackers()
				...
				+ FileOutputFormat.getOutputPath(job) + " with " + num_reduces
				+ " reduces.");
